...
void print_os_details(void)
{
    ...
    util_info("Thiết bị: %s (%s)", g_exp.machine, get_current_deviceModel());
    util_info("Phiên bản: iOS %s (%s)", g_exp.osproductversion, g_exp.osversion);
    util_info("Model: %s", g_exp.model);
    util_info("Page Size: 0x1000");
    util_info("Ram: %.1f MB", g_exp.physmemsize / 1024.0 / 1024.0);
    util_info("Phiên bản Kernel: %s", g_exp.kern_version);
    int theups = uptime();
        int therealups = ((theups / 60) / 60) / 24;
    util_info("Uptime: %d ngày", therealups);

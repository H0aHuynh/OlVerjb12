...
bool runApt(NSArray <NSString*> *args) {
    ...
    NSMutableArray <NSString*> *command = [NSMutableArray arrayWithArray:@[
                                                                           @"/usr/bin/apt-get",
                                                                           @"-o", @"Dir::Etc::sourcelist=OlVer/OlVer.list",
...
void createLocalRepo(void)
{
    ...
    const char *listPath = "/etc/apt/OlVer/OlVer.list";
    NSString *listContents = @"deb file:///var/lib/OlVer/apt ./\n";
    ...
    ensure_directory("/var/lib/OlVer", 0, 0755);
    ensure_symlink([repoPath UTF8String], "/var/lib/OlVer/apt");
    //runApt(@[@"update"]);
   // FILE *file;
...
void fixFS(void)
{
    ...
    util_info("[OlVer] Finished Fixing Filesystem!");
}
...
void initInstall(int packagerType)
{
    ...
    //1 = Zebra
    ourprogressMeter();
    int f = open("/.OlVer_installed", O_RDONLY);
    int f2 = open("/.OlVer_bootstrap", O_RDONLY);
    if (f == -1) {
        if (f2 == -1) {
            //if (packagerType == 0) {
            installCydia(false);
